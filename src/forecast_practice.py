...
load_df_clean[cols] = load_df_clean[cols].interpolate(
...
for col in cols_temp:
    temp_df_clean[col] = temp_df_clean[col].interpolate(
        method="linear", axis=0, limit_direction="both"
    )
...
load_long["date_time"] = pd.to_datetime(load_long[["year", "month", "day"]]) + pd.to_timedelta(
    load_long["hour"] - 1, unit="h"
)

# Keep only relevant columns and sort by date
load_long = (
    load_long[["zone_id", "date_time", "load"]]
...
# Create a datetime column
temp_long["date_time"] = pd.to_datetime(temp_long[["year", "month", "day"]]) + pd.to_timedelta(
    temp_long["hour"] - 1, unit="h"
)
...
merged_all_cleaned = merged_all.dropna().reset_index(
    drop=True
)  # drop rows with any NaNs and reset index
...
def _normalize_holiday_matrix(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _parse_row(row: Dict[str, Any]) -> pd.Timestamp:
...
def make_features(
    df: pd.DataFrame,
    holidays: Union[str, os.PathLike, pd.DataFrame],  # can be a path or a wide-format DataFrame
    *,
...
zone = 1
df_zone = (
    features_with_lags.loc[features_with_lags["zone_id"] == zone]
...
if len(y_clean) > H + 1:
    ...
    y_test, X_test = (
        y_clean.loc[cutoff + pd.Timedelta(hours=1) :],
        X_clean.loc[cutoff + pd.Timedelta(hours=1) :],
    )
...
X_all = ensure_calendar_features(
    df_zone.drop(columns=["zone_id", "load"], errors="ignore"), df_zone.index
)
...
# 3) Clean rows
mask = (
    y_all.notna() & np.isfinite(y_all) & X_all.notna().all(axis=1) & np.isfinite(X_all).all(axis=1)
)
y_clean = y_all[mask]
X_clean = X_all[mask]
...
print(
    f"[XGB] Holdout (168h): MSE={mse_te:.2f}  RMSE={rmse_te:.2f}  MAE={mae_te:.2f}  sMAPE={smape_te:.2f}%"
)
...
def _pair(f: str, X_shap: Any) -> str:
    if f == "hour_sin" and "hour_cos" in X_shap.columns:
        return "hour_cos"
    if f == "hour_cos" and "hour_sin" in X_shap.columns:
        return "hour_sin"
    if f == "doy_sin" and "doy_cos" in X_shap.columns:
        return "doy_cos"
    if f == "doy_cos" and "doy_sin" in X_shap.columns:
        return "doy_sin"
    return "auto"
...
plt.savefig("shap_importance_bar.png", dpi=160, bbox_inches="tight")
plt.figure()
shap.summary_plot(shap_values, X_shap, show=False)
plt.savefig("shap_beeswarm.png", dpi=160, bbox_inches="tight")
...
pred_y = float(
    xgb.predict(X_test.iloc[[idx]])[0] if hasattr(X_test, "iloc") else xgb.predict(X_test[[idx]])[0]
)
...
exp_worst = explainer_lime.explain_instance(
    data_row=instance_worst, predict_fn=xgb.predict, num_features=15
)
...
fig2.savefig(
    (
        f"lime_worst_error_zone1_{label_worst:%Y%m%d_%H%M}.png"
        if isinstance(label_worst, pd.Timestamp)
        else f"lime_worst_error_zone1_{label_worst}.png"
    ),
    dpi=200,
    bbox_inches="tight",
)
